...
function renderNFe(){
  ...
  document.getElementById('view').innerHTML=
   ...
   '<div class="panel"><h3>📋 OS aprovadas prontas para emitir</h3>'+
     (semNota.length?semNota.map(function(o){var v=veh(o.veiculoId);return '<div class="veh"><div class="plate">'+(v.placa||'')+'</div>'+
       '<div class="info"><div class="t">OS #'+o.numero+' · '+(cli(o.clienteId).nome||'')+'</div>'+
       '<div class="s">'+money(valorServicos(o))+' em serviços · '+(v.modelo||'')+'</div></div>'+
   ...
   '<div class="panel"><div class="head"><h3>🗂️ Notas registradas</h3><div class="sp"></div>'+
     ...
     (notas.length?'<table class="tbl"><thead><tr><th>Nº NFS-e</th><th>Tomador</th><th>Data</th><th>Valor</th><th></th></tr></thead>'+
       '<tbody>'+notas.map(function(n){return '<tr style="cursor:pointer" onclick="editarNota(\''+n.id+'\')"><td><b>'+(n.numero||'—')+'</b></td>'+
...
function prepararNFSe(osId){
  ...
  var opts=aprovadas.map(function(o){return '<option value="'+o.id+'"'+(o.id===osId?' selected':'')+'>OS #'+o.numero+' — '+(cli(o.clienteId).nome||'')+' — '+money(valorServicos(o))+'</option>';}).join('');
...
function editarNota(id){ var n=byId(nfList(),id); if(!n)return;
  modal("Editar nota","",
    '<label>Nº da NFS-e</label><input id="en_num" value="'+(n.numero||'').replace(/"/g,'&quot;')+'">'+
    ...
    '<label>Discriminação</label><textarea id="en_disc" rows="3">'+(n.discriminacao||'')+'</textarea>',
...
function verMinuta(id){ var n=byId(nfList(),id); var o=byId(WORK.os,n.osId)||{}; var v=veh(o.veiculoId)||{}; var f=fiscalCfg();
  modal("Minuta NFS-e"+(n.numero?(" nº "+n.numero):""),"Documento de controle — não substitui a nota oficial",
    '<div class="info-line"><span class="k">Prestador</span><span>'+(window.BRAND_NAME||'Vizio Motors')+' · '+(f.regime||'MEI')+'</span></div>'+
    '<div class="info-line"><span class="k">Tomador</span><span>'+n.cliente+'</span></div>'+
    ...
    '<div style="font-size:12.5px;color:var(--txt)">'+(n.discriminacao||discriminacao(o))+'</div>'+

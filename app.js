...
function emblemSVG(){return `<svg viewBox="0 0 100 100" width="100%" height="100%" style="max-width:110px" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><image x="1" y="1" width="98" height="98" href="vizio-symbol.png" xlink:href="vizio-symbol.png"/></svg>`}
...
function renderHome(){
  ...
  const alertas=[
    ['📦', critico.length?`<b>${critico.length} peça(s)</b> abaixo do mínimo: ${critico.map(p=>p.nome).join(', ')} — sugerir compra.`:'Estoque saudável — nenhuma peça abaixo do mínimo.'],
  ...
  document.getElementById('view').innerHTML=`
   <div class="panel"><div class="head"><h3>🔧 Ordens de Serviço</h3><div class="sp"></div>
     ...
     <tbody>${list.map(o=>{const v=veh(o.veiculoId);return `<tr onclick="openOS('${o.id}')">
       <td><b>#${o.numero}</b></td><td><span class="plate">${v.placa||''}</span></td>
       <td>${cli(o.clienteId).nome||''}</td><td><span class="badge s${o.statusIdx}">${STATUS_FLOW[o.statusIdx]}</span></td>
   ...
   <div class="osgrid" style="margin-top:14px">
    ...
    <div>
     <div class="panel">
       ...
       <div class="info-line"><span class="k">Cliente</span><span>${c.nome||''}</span></div>
       <div class="info-line"><span class="k">Veículo</span><span>${v.modelo||''}</span></div>
       <div class="info-line"><span class="k">Placa</span><span class="plate">${v.placa||''}</span></div>
       ...
       <div class="info-line"><span class="k">Responsável</span><span>${o.responsavel||'—'}</span></div>
     ...
     <div class="panel"><h3>Observações</h3><div style="font-size:13px;color:var(--txt);line-height:1.5">${o.obs||'—'}</div></div>
    </div>
   </div>`;
}
function itensHTML(o){return (o.itens||[]).map((it,i)=>{const ref=it.tipo==='servico'?svc(it.refId):prt(it.refId);
  return `<div class="itemrow"><div class="g">${it.tipo==='servico'?'🔧':'📦'} ${ref.nome||'—'} ${it.qtd>1?'× '+it.qtd:''}</div>
...
function addItem(id){const o=byId(WORK.os,id);
  const opts=`<optgroup label="Serviços">${WORK.servicos.map(s=>`<option value="servico:${s.id}">${s.nome} — ${money(s.preco)}</option>`).join('')}</optgroup>
   <optgroup label="Peças">${WORK.pecas.map(p=>`<option value="peca:${p.id}">${p.nome} — ${money(p.preco)}</option>`).join('')}</optgroup>`;
...
function novaOS(){
  modal("Nova Ordem de Serviço","Selecione cliente e veículo",`
    <label>Cliente</label><select id="f_cli" onchange="fillVeic()">${WORK.clientes.map(c=>`<option value="${c.id}">${c.nome}</option>`).join('')}</select>
...
function fillVeic(){const c=document.getElementById('f_cli').value;const sel=document.getElementById('f_vei');
  ...
  sel.innerHTML=vs.map(v=>`<option value="${v.id}">${v.placa} — ${v.modelo}</option>`).join('')||'<option value="">Sem veículo cadastrado</option>';}
function editOS(id){const o=byId(WORK.os,id);
  modal("Editar OS","#"+o.numero,`
    <div class="frow"><div><label>Responsável</label><input id="f_resp" value="${o.responsavel||''}"></div>
    <div><label>Previsão</label><input id="f_prev" type="date" value="${o.previsao||''}"></div></div>
    <label>Observações</label><textarea id="f_obs">${o.obs||''}</textarea>`,
...
function openCli(id){const c=byId(WORK.clientes,id);const vs=WORK.veiculos.filter(v=>v.clienteId===id);
  ...
  document.getElementById('view').innerHTML=`
   ...
   <div class="osgrid" style="margin-top:14px">
     <div>
       <div class="panel"><div class="head"><h3>Veículos de ${c.nome}</h3><div class="sp"></div><button class="b b-sm" onclick="novoVeic('${id}')">+ Veículo</button></div>
         ${vs.map(v=>`<div class="veh"><div class="plate">${v.placa}</div><div class="info"><div class="t">${v.modelo}</div>
           <div class="s">${v.ano} · ${(v.km||0).toLocaleString('pt-BR')} km · ${v.cor} · ${v.combustivel}</div></div>
     ...
     <div class="panel"><h3>Ficha</h3>
       <div class="info-line"><span class="k">Telefone</span><span>${c.tel||'—'}</span></div>
...
function formCli(c){c=c||{};const ed=!!c.id;
  modal(ed?"Editar cliente":"Novo cliente","",`
    <label>Nome</label><input id="f_nome" value="${c.nome||''}">
    <div class="frow"><div><label>Telefone</label><input id="f_tel" value="${c.tel||''}"></div>
    <div><label>Aniversário</label><input id="f_nasc" type="date" value="${c.nasc||''}"></div></div>
    <label>E-mail</label><input id="f_email" value="${c.email||''}">
    <label>Observações</label><textarea id="f_obs">${c.obs||''}</textarea>`,
...
function renderEstoque(q){q=(q||'').toLowerCase();
  ...
  document.getElementById('view').innerHTML=`
   <div class="panel"><div class="head"><h3>📦 Estoque Inteligente</h3><div class="sp"></div>
     ...
     <tbody>${list.map(p=>{const low=p.estoque<p.minimo;return `<tr><td><b>${p.nome}</b></td><td style="color:var(--muted)">${p.fornecedor}</td>
...
function renderPortal(token){
  ...
  P.innerHTML=`<div class="pcard">
    ...
    <div class="card-glass pbig">
      ...
      <div class="plate" style="margin:8px 0;font-size:15px">${v.placa||''}</div>
